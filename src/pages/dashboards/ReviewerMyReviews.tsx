...
import { FileText, CheckCircle, Clock, Eye, X, MessageSquare, AlertCircle, Calendar } from 'lucide-react';
...
export default function ReviewerMyReviews() {
  ...
  const filteredAssignments = assignments.filter((item) => {
    const isCompleted = item.status === 'completed' || (item.reviews && item.reviews.length > 0);
    ...
    <DashboardLayout>
      <div className="max-w-5xl">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-serif font-bold text-academic-900 mb-2">Review Saya</h1>
            <p className="text-academic-500">Daftar review yang telah ditugaskan dan disubmit oleh Anda.</p>
          </div>
        </div>

        {/* Tab Filters */}
        <div className="flex border-b border-academic-200 mb-6 gap-2">
          <button
          ...
          >
            Menunggu Review ({assignments.filter(a => a.status !== 'completed' && !(a.reviews && a.reviews.length > 0)).length})
          </button>
          <button
          ...
          >
            Selesai ({assignments.filter(a => a.status === 'completed' || (a.reviews && a.reviews.length > 0)).length})
          </button>
        </div>
        ...
        <div className="bg-white rounded-xl border border-academic-200 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-academic-500 flex flex-col items-center justify-center gap-2">
              ...
              <p>Belum ada data review pada kategori ini.</p>
            </div>
          ) : (
            <div className="divide-y divide-academic-100">
              {filteredAssignments.map((assign: any) => {
                ...
                const isCompleted = assign.status === 'completed' || hasReviewed;
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-academic-500">
                          ...
                          {assign.due_date && (
                            <span className={`flex items-center gap-1 ${
                              new Date(assign.due_date) < new Date() && !isCompleted
