...
export default function RegisterEditor() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("STEP 5 INSERT EDITOR PROFILE");
      const { error: profileError } = await supabase.from('editor_profiles').insert({
        user_id: authData.user.id,
        affiliation: formData.institution,
        faculty: formData.faculty,
        education_level: formData.education_level,
        expertise_area: formData.expertise_area,
        editorial_experience: formData.editorial_experience,
      ...
      <main className="flex-grow max-w-2xl mx-auto px-4 py-12 w-full">
        <div className="bg-white rounded-xl border border-academic-200 shadow-sm overflow-hidden">
          <div className="bg-brand-900 p-6 text-center">
            <h1 className="text-2xl font-serif font-bold text-white mb-2">Registrasi Editor</h1>
            <p className="text-brand-100 text-sm">Daftar sebagai Dewan Redaksi (Editor)</p>
          </div>
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold text-academic-700 uppercase mb-2">Pengalaman Editorial *</label>
                <textarea required value={formData.editorial_experience} onChange={e => setFormData({...formData, editorial_experience: e.target.value})} rows={3} className="w-full border border-academic-300 rounded-md px-4 py-2 text-sm focus:ring-brand-500" placeholder="Ceritakan pengalaman Anda sebagai Editor..."></textarea>
              </div>
              ...
              <button type="submit" disabled={loading} className="w-full bg-brand-600 hover:bg-brand-700 text-white font-bold py-3 rounded-lg transition-colors mt-6 disabled:opacity-50">
                {loading ? 'Memproses...' : 'Daftar Sebagai Editor'}
              </button>
            </form>
          </div>
        </div>
